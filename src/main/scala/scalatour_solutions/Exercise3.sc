...
val words: Array[String] = lines.flatMap(_.split("""\s+"""))
...
def removeNonLetters(word: String): Option[String] = {
	val filtered = word.filter(_.isLetter)
	if (filtered.isEmpty) {
		None
	} else {
		Some(filtered)
...
// than lowerWords
val cleanLowerWords = lowerWords.flatMap(removeNonLetters)
...
var wordCountMap: Map[String, Int] = Map[String, Int]().withDefaultValue(0)

cleanLowerWords.foreach { word =>
	val currCount = wordCountMap(word)
	val newCount = currCount + 1
	wordCountMap = wordCountMap + (word -> newCount)
}


// 6. Do the word count using the groupBy method and mapValues
// on the result.
val wordToWordsMap: Map[String, Array[String]] =
	cleanLowerWords.groupBy(identity(_))

val wordCountMap2 = wordToWordsMap.mapValues(words => words.length)
