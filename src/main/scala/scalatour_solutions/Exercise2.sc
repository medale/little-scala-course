...
case class Runtime(hours: Int, min: Int, sec: Int) {
	Runtime.validate(hours, min, sec)
}
...
val RuntimeRegex = """.*?Runtime: (\d\d*) hours : (\d\d*) min : (\d\d*) sec""".r
...
def getRuntime(logEntry: String): Either[String, Runtime] = {
	val result: Either[String, Runtime] = logEntry match {
		case RuntimeRegex(hours, min, sec) => try {
			val runtime = Runtime(hours.toInt, min.toInt, sec.toInt)
			Right(runtime)
		} catch {
			case e: Exception => {
		...
		case _ => Left(s"Non-runtime: ${logEntry}")
	}
	result
}
